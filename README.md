...
When the logistic model log odds coefficients are converted to odds (i.e. e<sup>(log odds)</sup>) and then odds are distinguished in terms of odds survived (i.e. if e<sup>(log odds)</sup> >= 1, then e<sup>(log odds)</sup>) and odds not survived (i.e. if e<sup>(log odds)</sup> < 1, then 1 / e<sup>(log odds)</sup>), the following observations can be made. For example, as the independent variable Male increases by one unit, the odds that the person did not survive is multipled by over 10 (Pardoe, 2012, p.271). 
